...
const DEFAULT_CATEGORIES = [
  "News", "Local", "Crime", "Politics", "Sports", "Business", 
  "Entertainment", "Technology", "Health", "Education",
  "Automotive", "Real Estate", "Lifestyle", "Food", 
  "Music", "Trending", "Astro", "Career", "Agriculture", "Lottery", "Obituary"
];
...
export default function CategoryMenu() {
  ...
  const handleCategoryHover = async (cat: string) => {
    ...
    setIsLoading(true);
    try {
      const data = await fetchArticles(cat, "", 1, 3, "published", "All Places");
  ...
  const activeArticles = hoveredCategory ? categoryDataCache[hoveredCategory] || [] : [];
  ...
  return (
    ...
    >
      ...
      <div className="w-full px-4 h-12 flex items-center justify-between relative max-w-[1600px] mx-auto">
        ...
        <div className="relative flex-shrink-0 pl-3 border-l border-gray-200 dark:border-gray-700 ml-1 h-full flex items-center" ref={dropdownRef}>
          <button 
            ...
            {isLoading && activeArticles.length === 0 ? (
              ...
              <div className="grid grid-cols-4 gap-8">
                ...
                <div className="flex flex-col justify-between border-r border-gray-100 dark:border-gray-800 pr-6">
                  <div>
                    <span className="text-[10px] font-bold text-[#e3000f] uppercase tracking-widest">Category</span>
                    <h3 className="font-black text-2xl text-gray-900 dark:text-white uppercase tracking-tight mt-1">{hoveredCategory}</h3>
                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">
                      Catch up on the latest local reports and updates for {hoveredCategory}.
                    </p>
                  </div>
