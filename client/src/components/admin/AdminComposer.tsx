...
import { AlertCircle, Loader2, Globe, EyeOff, Plus, Trash2 } from "lucide-react";
...
interface AdminComposerProps {
  ...
  defaultMode: "news" | "shorts" | "ad" | "obituary"; // <-- Added obituary
  initialShortUrl: string;
  onCancel: () => void;
  onSuccess: () => void;
}

// NEW: Interface for tracking multiple photos
interface MediaItem {
  file: File | null;
  url: string;
  credit: string;
...
export default function AdminComposer({
  ...
  const [editorMode, setEditorMode] = useState<"news" | "shorts" | "ad" | "obituary">(defaultMode); // <-- Added obituary
  const [shortUrl, setShortUrl] = useState(initialShortUrl);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [error, setError] = useState("");

  // NEW: State for tracking multiple images and their credits
  ...
  useEffect(() => {
    ...
    // If editing, load the existing array of images into our state
  ...
  // NEW: Handlers for the Multi-Photo array
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (editorMode === "shorts") {
        ...
        // Ads just use the first item in the media list
        if (shortUrl) {
          const youtubeId = shortUrl.split("/").pop()?.split("?")[0];
        ...
        // --- NEWS / OBITUARY MODE: Upload Multiple Images ---
        for (const item of mediaList) {
          if (item.file) {
            ...
            // Keep existing image but update credit if they typed a new name
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="bg-white dark:bg-[#111] border border-gray-200 dark:border-gray-800 rounded-2xl shadow-sm p-6 md:p-8">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* Hides toggles if editing, or in ad/obituary mode */}
          ...
          <div>
            ...
            <input
              ...
              placeholder={
                editorMode === "shorts" ? "Catchy video title..." : 
                editorMode === "ad" ? "e.g., Summer Sale Promo..." : 
                editorMode === "obituary" ? "e.g., John Doe (85), beloved father..." : 
          ...
          {editorMode === "shorts" ? (
            ...
            <>
              ...
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-2">
                  {/* UPDATED: Dynamic Label based on mode */}
                  {editorMode === "obituary" ? "Submitted By / Family Contact" : "Reported By"}
                </label>
                <input
                  type="text"
                  name="reportedBy"
              ...
              {/* --- MULTIPLE PHOTOS WITH INDIVIDUAL CREDITS --- */}
              <div className="space-y-4">
                <label className="block text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-2">
                  Media Attachments & Photographer Credits
                </label>
                ...
                <div className="space-y-3">
                  {mediaList.map((media, index) => (
                    <div key={index} className="p-4 bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl relative group transition-all">
                      ...
                      {/* Delete Button for extra photos */}
                      {mediaList.length > 1 && (
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
                        ...
                        <div className="w-full">
                          ...
                          <input type="text" value={media.credit} onChange={(e) => handleMediaChange(index, "credit", e.target.value)} placeholder="e.g., Staff Photographer" className="w-full p-2.5 bg-white dark:bg-[#111] border border-gray-200 dark:border-white/10 rounded-lg text-sm focus:ring-2 focus:ring-red-600 outline-none" />
                        </div>
                      </div>
                    </div>
                  ))}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-8 pt-6 border-t border-gray-100 dark:border-white/10">
                <div>
                  ...
                  {/* UPDATED: Disabled and locked to Obituary if editorMode is obituary */}
                  <select 
                    name="category" 
